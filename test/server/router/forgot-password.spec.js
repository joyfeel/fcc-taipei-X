...
describe('/forgot_password', () => {
  ...
  const notExistAccount = {
    email: "notexist@gmail.com"
  }

  const emailToken = getToken['EMAIL'](existAccount.email)
  // 1. DB那要有資料
  // 2. joybee210-FAKE@gmail.com 忘記密碼
  // 3. joybee210@gmail.com 忘記密碼

  it('normal signup', async done => {
    const user = new User(userSignupInfo)
    await user.save()
    done()
  })

  it('abnormal forgot-password [wrong email]', async done => {
    request()
  ...
  it('abnormal forgot-password [No such account]', async done => {
    request()
      .post('/v1/forgot_password')
      .send(notExistAccount)
      .expect('Content-Type', 'application/json; charset=utf-8')
      .expect(404)
      .expect({
        status: 'error',
  ...
  it('normal forgot-password', function(done) {
  ...
  //After the user click the URL
  it('Try to get user information', done => {
    request()
      .get('/v1/signup')
  ...
  it.only('normal reset password', done => {
    request()
